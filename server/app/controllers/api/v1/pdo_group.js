...
module.exports = function(router) {
    router({
        ...
        pdo_group.pdos = JSON.parse(req.body.pdos).map(function(pdo) {
    ...
    router({
        ...
        PdoGroup.findById(req.params.pdo_group_id, 'comments', function(err, pdo_group) {
            if (err) {
                res.send(err);
                return;
            }
            ...
            res.send(pdo_group.comments.id(req.params.comment_id).resource(req.route_gen));
        });
    });
    router({
