...
describe('Test de grupos de PDOs', function() {
        ...
        mongoose.connection.once('connected', function() {
            q.all([
                ...
                q.all(prom_array).then(function(results) {
                    created_pdo = results.slice(0, 2).map(function(e) {
                        return e.body;
                    });
                    pdo_to_add_later = results[2].body;
                    pdo_other_school = results.slice(3).map(function(e) {
                        return e.body;
                    });
                    done();
                }).
    ...
    it('should create a pdo_group', function(done) {
        ...
        pdo_group.pdos = created_pdo.map(function(pdo) {
            return pdo._id;
        });
        request(url).
    ...
    it('should remove a pdo from a group', function(done) {
        var prom;
        prom = request(url).
        delete('/api/v1/pdo_group/' + pdo_group_created._id + '/pdo/' + pdo_to_add_later._id).
        set(token).
    ...
    it('should not group pdos from another school', function(done) {
        ...
        pdo_group.pdos = pdo_other_school.map(function(pdo) {
            return pdo._id;
        });
        pdo_group.pdos.push(pdo_to_add_later._id);
        var prom = request(url).
        post('/api/v1/pdo_group').
        set(token).
        send(pdo_group).
        expect('Content-Type', /json/).
